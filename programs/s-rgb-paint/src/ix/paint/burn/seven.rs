...
use crate::BurnPixelsSeven;
use crate::ix::paint::burn::almost_burn;

pub fn ix(ctx: Context<BurnPixelsSeven>) -> Result<()> {
    ...
    // burn pixel seven
    burn(
        &ctx.accounts.pixel_seven_mint,
        &ctx.accounts.pixel_seven_mint_ata,
        |p| &p.burned.plan.five,
    )?;
    // proof
    let proof = &mut ctx.accounts.proof;
    assert_eq!(proof.arity, 7);
