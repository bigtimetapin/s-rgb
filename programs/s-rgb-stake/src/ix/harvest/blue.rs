...
use crate::{HarvestBlue, pda};

pub fn ix(ctx: Context<HarvestBlue>) -> Result<()> {
    // get accounts
    ...
    let harvest_cpi_context = CpiContext::new(
        ctx.accounts.token_program.to_account_info(),
        MintTo {
            mint: ctx.accounts.blue_mint.to_account_info(),
            to: ctx.accounts.blue_mint_ata.to_account_info(),
